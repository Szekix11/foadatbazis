  ...
  function view(data, request, id){
    ...
    if (request.status >= 200 && request.status < 400) {
         data.forEach((query) => {
          ...
          div.innerHTML = "<td>"+query.id+"</td><td><input id='name"+query.id+"' placeholder='"+query.name+"' value='"+query.name+"'/></td><td><input id='email"+query.id+"' placeholder='"+query.email+"' value='"+query.email+"'/></td><td><input id='address"+query.id+"' placeholder='"+query.address+"' value='"+query.address+"'/><td><input id='phonenumber"+query.id+"' placeholder='"+query.phonenumber+"' value='"+query.phonenumber+"'/><td><input id='ime"+query.id+"' placeholder='"+query.ime+"' value='"+query.ime+"'/><td><input id='"+query.id+"' placeholder='"+query.buy+"' value='"+query.buy+"'/><td><input id='phonetype"+query.id+"' placeholder='"+query.phonetype+"' value='"+query.phonetype+"'/></td>"+"<button onclick = 'deleterecord("+query.id+")' type = 'submit' value='Submit'>Delete</button>"+"<button onclick = 'update("+query.id+")'>Update</button>" ;
...
function update(id){
  ...
  navigator.sendBeacon('http://127.0.0.1:5000/updatedetails/', data);    address:document.getElementById("address"+id).value,
